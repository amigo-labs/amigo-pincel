...
_Last updated: 2026-05-12_ (M8.7a: Tileset Panel — new Svelte component `ui/src/lib/components/TilesetPanel.svelte` mounted as a right-side sidebar in `App.svelte`. Lists each tileset (name, id, tile_w × tile_h, tile_count) via the M8.6 wasm surface (`tilesetCount`, `tilesetIdAt`, `tilesetName`, `tilesetTileWidth`, `tilesetTileHeight`, `tilesetTileCount`) and exposes an inline "Add Tileset" form that calls `addTileset(name, tile_w, tile_h)`. The form validates non-empty names and positive-integer tile sizes locally, then surfaces any wasm `Result::Err` (id-space exhaustion, command-bus rejection) inline as a `role="alert"` message. Reactivity over the opaque wasm getters is driven by a `tilesetRev: number` `$state` counter in `App.svelte` that bumps on every event the panel cares about: `newDoc`, `openFile`, `undo`, `redo`, and the panel's own `onChange` callback after a successful add. No new wasm surface, no new Rust code, no new deps. **M8.7b–d still pending** — per-tile thumbnails, Tilemap Stamp tool + active-layer concept, and the Tileset Editor sub-mode each add new wasm surface and stay their own M-sized commits. Headless dev-server probe was blocked by the pre-existing `vite.config.ts` `server.fs.allow` (limits serving to `../crates/pincel-wasm/pkg`, excludes `ui/index.html`); `pnpm check`, `pnpm lint`, `pnpm build`, and `cargo check --workspace` all green. Visual verification in a human-driven `pnpm dev` session is deferred to follow-up.)

_Previously (2026-05-11)_: M8.6: pincel-wasm tilemap surface — `Document.addTileset(name, tileW, tileH) -> tilesetId` (assigns `max(existing) + 1`, routes through the `AddTileset` command so it joins the undo bus), `Document.placeTile(layer, frame, gridX, gridY, tileId)` (routes through `PlaceTile`, emits a `dirty-rect` event sized to the parent layer's `tile_size`), and a set of read-only getters: `tilesetCount`, `tilesetIdAt(index)`, `tilesetName(id)`, `tilesetTileWidth(id)`, `tilesetTileHeight(id)`, `tilesetTileCount(id)`. Flip / rotate flags on placeTile are intentionally not yet surfaced — the UI can wire them once M8.7 needs them. 9 new pincel-wasm unit tests pin id assignment, undo-bus join, getter round-trip, dirty-rect emission shape, and the non-tilemap-layer / out-of-grid rejection paths. **M8.1–M8.6 complete**; M8.7 (Tileset Panel + Tilemap Stamp tool + Tileset Editor sub-mode) is L-sized per CLAUDE.md §3.2 and deferred to follow-up sessions — it needs to split into ~3-4 M-sized commits and adds new wasm surface (`tilePixels`, `paintTilePixel`) for per-tile editing. The marketing-site Cloudflare Workers Builds deploy that landed on main alongside this work is preserved under "Website — Cloudflare Workers Builds deploy" below.)
...
- [ ] **M8.7** — UI: Tileset Panel (palette of tile thumbnails),
  ...
  - [x] **M8.7a** — Tileset Panel (Svelte component listing existing
