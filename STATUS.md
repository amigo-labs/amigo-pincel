...
**Branch:** `claude/continue-from-status-T3QCr` · M10.1 complete
(File System Access API open / save adapter with `<input>` +
download fallback; toolbar label switches `Save` vs `Save As
(download)` per capability; `Save As…` button on FSA-capable
browsers).

## Next task

**M10.2** — IndexedDB substrate: per-document UUIDs, recent-files
registry, settings / prefs store. Lays the data substrate that
M10.3 (autosave timer + recovery dialog) and M10.4 (service
worker + web app manifest + icons) build on. See CLAUDE.md §4 /
spec §10.3.
