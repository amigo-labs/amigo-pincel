...
Pincel is a pixel-art editor at the feature level of Aseprite, focused on game asset creation — and, since M16, a paint.net-style image editor in the same app. It ships as a PWA and a native Tauri app sharing one Svelte UI and two Rust cores: `pincel-core` behind **Pixel mode** and `fineliner-core` (the former sister project `amigo-fineliner`) behind **Image mode**. The mode follows the document (`.aseprite` → Pixel; PNG / JPEG / WebP / BMP / GIF / TIFF → Image). The two cores stay separate on purpose — see the spec's Decision Log, 2026-09-06 "Two cores, one shell". `crates/pincel-effects` serves both.
...
### 5.3 `pincel-wasm` and `fineliner-wasm`

- **Crate type:** `cdylib`. Built via `wasm-pack build --target web --release` (`pnpm wasm:build` in `ui/` builds both).
- **Public API:** `pincel-wasm` matches the npm-package surface in spec §17.5. Keep it small and stable. `fineliner-wasm` is handle-based with a JSON `CommandSpec` / `EffectSpec`; its TypeScript mirror under `ui/src/modes/image/core/generated/` is generated by `cargo test -p fineliner-wasm export_command_spec` and committed (CI fails on drift).
...
- **Image-mode core conventions** (`fineliner-core`): same rules as §5.1 — no platform deps, no system-level file I/O, `thiserror` errors, `std`-only. The `image` crate is its single codec dependency.
...
- **WASM bridge:** all Pixel-mode calls go through the `src/lib/core/` adapter and all Image-mode calls through `src/modes/image/core/wasm.ts`; UI never imports `pincel-wasm` / `fineliner-wasm` directly.
- **Two modes, one shell:** `src/App.svelte` owns the start screen, the New dialog, file → mode routing (`src/lib/fs` `sniffFormat`) and mounts exactly one editor per session — `src/modes/pixel/PixelEditor.svelte` or `src/modes/image/ImageEditor.svelte` — via dynamic `import()`. Editors talk to the shell only through `EditorProps` (`src/lib/shell/types.ts`). Shared layers (`src/lib/fs`, `src/lib/idb`, `src/lib/menu`, `src/lib/platform`) stay mode-agnostic.
...
cargo clippy -p pincel-core -p pincel-effects -p aseprite-writer -p pincel-wasm -p fineliner-core -p fineliner-wasm --all-targets -- -D warnings
cargo fmt
...
# pincel-tauri in a dedicated job; scope local clippy/test to the six
# library crates as above.

# Image mode: the TypeScript mirror of fineliner-wasm's CommandSpec /
# EffectSpec is generated by a test and committed; CI fails on drift.
cargo test -p fineliner-wasm export_command_spec   # writes ui/src/modes/image/core/generated/
```
...
```bash
cd ui
pnpm wasm:build                              # build both wasm pkgs (pincel-wasm, fineliner-wasm) — required before install
pnpm install
...
```bash
cargo fmt && cargo clippy -p pincel-core -p pincel-effects -p aseprite-writer -p pincel-wasm -p fineliner-core -p fineliner-wasm --all-targets -- -D warnings && cargo test -p pincel-core -p pincel-effects -p aseprite-writer -p pincel-wasm -p fineliner-core -p fineliner-wasm && cd ui && pnpm lint && pnpm check && pnpm build
```
...
crates/pincel-core/           Pixel-mode core: pure logic, no I/O, no platform
crates/fineliner-core/        Image-mode core (from amigo-fineliner): layers, brushes, codecs
crates/pincel-effects/        Stateless effects + adjustments, shared by both cores
crates/aseprite-writer/       Standalone, publishable, MIT/Apache
crates/pincel-wasm/           wasm-bindgen layer for pincel-core, cdylib
crates/fineliner-wasm/        wasm-bindgen layer for fineliner-core, cdylib

ui/                           Svelte 5 + Vite frontend (PWA + Tauri)
ui/src/App.svelte             Two-mode shell: start screen, New dialog, mode routing
ui/src/lib/shell/             Shell contract (EditorProps), session store, start screen, New dialog
ui/src/modes/pixel/           PixelEditor.svelte (toolbar, canvas, tool input, state)
ui/src/modes/image/           ImageEditor.svelte + its stores / controller / components
ui/src/lib/core/              WASM adapter (Pixel mode)
ui/src/modes/image/core/      WASM adapter (Image mode) + generated ts-rs types
...
*Last updated: 2026-09-07. Amend in place via PR. Significant changes get a Decision Log entry in `docs/specs/pincel.md` §15.*
